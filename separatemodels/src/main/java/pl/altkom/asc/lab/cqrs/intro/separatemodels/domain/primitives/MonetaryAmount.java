package pl.altkom.asc.lab.cqrs.intro.separatemodels.domain.primitives;
...
public class MonetaryAmount implements Comparable<MonetaryAmount> {
    ...
    public MonetaryAmount(BigDecimal amount) {
        this.amount = new BigDecimal(String.valueOf(amount), new MathContext(2, RoundingMode.HALF_UP));
    }
    ...
    public static MonetaryAmount from(Integer amount) {
        if (amount == null) {
            throw new RuntimeException("Amount for MonetaryAmount cannot be null");
        }
        return new MonetaryAmount(new BigDecimal(amount));
